...
if (!isset($lj_options)) 
{
	$lj_options = array (	
			  ...
			  "id" => $wp_lj_plugin['slug']."_adminbar",
			  "default" => '', 	
			  "input_type" => "button",
...
if(!function_exists('lj_admin_styles'))	
{
	function lj_admin_styles()
	{
		...
		if(is_admin() || lj_is_login_page())	{
			wp_register_style( $wp_lj_plugin['slug'], $wp_lj_plugin['url'] . 'assets/default-css/style.css','',$wp_lj_plugin['version'] );
		}
		 wp_register_style( $wp_lj_plugin['slug'].'_admin_bar', $wp_lj_plugin['url']. 'assets/default-css/adminbar.css','', $wp_lj_plugin['version'] );
...
if(!function_exists('lj_bd_admin'))
{
	function lj_bd_admin()	{	
		global $wp_lj_plugin;
		add_menu_page($wp_lj_plugin['name'],$wp_lj_plugin['name'], 'add_users', $wp_lj_plugin['slug'], 'lj_bd_admin_header','');
	}
...
if(!function_exists('lj_admin_footer'))	
{
	function lj_admin_footer(){
		echo '<span id="footer-thankyou">Thank you for creating with <a href="http://wordpress.org/">WordPress</a>. This theme developed by <a href="http://lineshjose.com/">Linesh Jose</a>.</span>';
	}
