<?php
	...
	if (!isset($lj_options)) {
		$lj_options = array (	
							...
							"default" => '', 	"input_type" => "button",	"settings_type"=>'common', "learn_more"=>'http://bit.ly/Pmrmas'),
							
							array(  
							"name" => "Background image:",	"desc" => "You can set custom background image. Please upload an image.",
							"id" => $wp_lj_plugin['slug']."_bg_image",	"default" => '', 
							"input_type" => "file",	"settings_type"=>'bg_img'),						
							
							array(  
							"name" => "Background Color:",	"desc" => "Please select a color to set as your bakground Color.",
							"id" => $wp_lj_plugin['slug']."_bg_color",	"default" => '#FFFFFF', 
							"input_type" => "text",	"settings_type"=>'bg_color'),
							
							array(  
							"name" => "Background position:",	"desc" => "Please select a color to set as your bakground Color.",
							"id" => $wp_lj_plugin['slug']."_bg_position",	
							"default" => array(array("Position",'top left'),array("Repeat",'repeat'),array("Attachment",'scroll')),	
							"input_type" => "radio",	"settings_type"=>'bg_pos'),
							
							array(  
							"name" => "Foreground Colors",	"desc" => "Please select a color to set as your bakground Color.",
							"id" => $wp_lj_plugin['slug']."_fg_colors",	
							"default" => array(array("reset",'yes'), array("Text",'#333333'),array("Link",'#3B5998')),	
							"input_type" => "text",	"settings_type"=>'fg_prop'),
						);
	}
	...
	// Checking advanced features is active //
	if(!function_exists('lj_feat_active'))	
	{
		function lj_feat_active()
		{ 
		global $lj_options,$wp_lj_plugin; 
		$val=get_settings($wp_lj_plugin['slug']."_cstyle");
	...
	// Function for filter background properties like position, repeat etc.. //
	if(!function_exists('lj_bg_feat'))	
	{
		function lj_bg_feat($args=array('',''))
		{ 
			$bk_img_pos= array( 'position'=> array('top left','top center','top right', 'center left','center center','center right','bottom left','bottom center','bottom right'), 'repeat'=> array('repeat-x','repeat-y','repeat','no-repeat'), 'attachment'=>array('scroll','fixed','strech' ));	
			if($args)
			{ 
				$i=1;
				foreach($bk_img_pos[strtolower($args[0])] as $value)
				{
					if($args[0]=='Position' && $i==3){$break="<p></p>"; $i=0;}else{$break='';}
					if($value==$args[1]) { $checked='checked '.$args[1];} else{ $checked='';}
					echo '<label><input name="background-'.strtolower($args[0]).'" type="radio" value="'.$value.'" '. $checked.' > '.$value.'</label> '.$break;
					$i++;
				}	
			}
		}
	...
	if(!function_exists('lj_delete_file'))
	{
		function lj_delete_file($file)
		{
			global $wp_lj_plugin;
			if (file_exists($file))
			{
				if(unlink($file)){if(update_option($wp_lj_plugin['slug']."_bg_image",'')) {$error='';}}
				else{$error=1;}
			}else{$error=1;}
		}
	}
	...
	// Fucntions to create dynamic CSS content */
	if(lj_feat_active() )
	{ 
		// Background 
		function lj_make_bg_css()
		{	
			global $wp_lj_plugin;
			$img_pos = get_settings($wp_lj_plugin['slug']."_bg_position");
				$css="\n\t".'background:'.get_settings($wp_lj_plugin['slug']."_bg_color").' url(\''.get_bloginfo('url').'/wp-content/uploads/'.get_settings($wp_lj_plugin['slug']."_bg_image").'\') ';
				foreach (get_settings($wp_lj_plugin['slug']."_bg_position") as $value){	
				if($value[1]=='strech'){$prop= 'fixed'; $cover="\t".'-webkit-background-size: cover;'."\n\t".'-moz-background-size: cover;'."\n\t".'-o-background-size: cover;'."\n\t".'background-size: cover;'; } else{$cover=''; $prop=$value[1];}
				$css.=$prop.' ';
				}
				$css.=';'."\n".$cover;
				return $css;
		...
		// Foreground
		function lj_make_fg_css()
		{	
			global $wp_lj_plugin;
			
			$col_opt=get_option($wp_lj_plugin['slug'].'_fg_colors');
			if($col_opt[0][0]=='reset' && $col_opt[0][1]=='no')
			{
				foreach($col_opt as $fg_prop)
				{
					if($fg_prop['0']=='Text')
					{
						$css ='html, body, div, span, p, blockquote, pre,  abbr, acronym,  code,  em, img,  small,   dl, dt, dd, ol, ul, li, fieldset, form, label, legend, table, caption, tr, th, td,  embed, h1,h2{color:'.$fg_prop[1].'!important}';
					}
					if($fg_prop['0']=='Link')
					{
						$css.= "\n".'
a{color:'.$fg_prop[1].'!important}
.delete a,.trash a,.del-link{color:red!important;}';
					}
				}
				$css.='';
			}
		...
		// Show CSS
		function lj_make_css()
		{	
			echo "\n".'<style>'."\n".'body'."\n".'{'.lj_make_bg_css()."\n".'}'."\n".lj_make_fg_css()."\n".'</style>'."\n";
		}
		add_action('admin_head', 'lj_make_css');
	}
